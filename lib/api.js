...
const ROUTES = [
  ...
  { m: 'post', re: /^\/groups\/create$/, h: (_, body) => ok({ id: 'g' + Date.now(), adminId: 'u1', deactivatedMemberIds: [], memberIds: ['u1'], taskIds: [], ...body }) },
...
if (USE_DUMMY_DATA) {
  API.defaults.adapter = async (config) => {
    ...
    // Not mocked yet — fall through to the real network call.
